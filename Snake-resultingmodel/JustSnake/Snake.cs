...
namespace NeuralNetworkSnake
{
    class Snake
    {
        ...
        public const int InitialTimeRemaining = 1000;
        public const int BonusTime = 1000;
        ...
        static int[][] FourDirections { get; } = { UP, RIGHT, DOWN, LEFT };
        static int[][] EightDirections { get; } = { UP, UPRIGHT, RIGHT, DOWNRIGHT, DOWN, DOWNLEFT, LEFT, UPLEFT };
        ...
        public void Step()
        {
            ...
            for (int i = 1; i < 4; i++)
                if (vib[i] > vib[MaxIndex])
                    MaxIndex = i;
            int[] dir = FourDirections[MaxIndex];
            Move(dir);
        }
        ...
        public void Draw(Graphics g)
        {
            ...
            g.FillEllipse(Brushes.Red, Apple[0] * Size, Apple[1] * Size, Size - 1, Size - 1 );
            foreach (int[] pos in body)
                g.FillRectangle(Brushes.Green, pos[0] * Size, pos[1] * Size, Size - 1, Size - 1);
            g.DrawString($"Нынешний счет: {Apples}\nНаилучший счет: {Form1.maxscore}", new Font("Helvetica", 40), Brushes.White, new Point(2, 2));
        }
    }
}
